...
image_dir  = Path('/home/kctd/disk/ccs/scaling/data/images/base64_images')
image_data = {}
image_names = []
...
for img in os.listdir(str(image_dir)):
    ...
    with open(str(full_path), 'r') as f:
        ...
        logging.info(f"Loaded image: {img}")
...
class SocialMediaUser(FastHttpUser):
    host = "http://10.98.251.63:8080"
    # host= "10.112.188.4:30082"
    def wait_time(self):
        global intervals
        global mean_iat
    ...
    def compose_post(self):
        ...
        for _ in range(5):
            if random.random() <= 0.2:
                ...
                for _ in range(num_urls):
                    text += f" https://www.bilibili.com/av{random_decimal(8)}"

        # Media
        num_media = random.randint(1, 4) if random.random() < 0.25 else 0
        num_media = 0
        media_names = []
        medium = []
        media_types = []
        ...
        media_names_str = ' '.join(media_names)
        ...
        # Send POST request
        try:
            r = self.client.post('/wrk2-api/post/compose', data=body)
            # print(r.text)
            if r.status_code > 202:
                logging.warning('compose_post response status = %d, text=%s' % (r.status_code, r.text))
        except Exception as e:
            logging.error(f"Error in compose_post: {str(e)}")
...
RPS = list(map(lambda x: int(x) //10, Path('trace.txt').read_text().splitlines()))
# RPS = list(map(lambda x: int(x) , Path('trace.txt').read_text().splitlines()))


class CustomShape(LoadTestShape):
    time_limit = len(RPS) #3600秒
    spawn_rate = 1 #每秒增加速度
